...
operations C<pth_cancel>, C<pth_raise>, C<pth_suspend>, C<pth_resume> and
C<pth_yield>(I<target>) also work across schedulers, but as I<asynchronous
...
C<pth_raise>(3), C<pth_suspend>(3), C<pth_resume>(3) and
...
C<pth_cancel>/C<pth_raise>/C<pth_suspend>/C<pth_resume>/C<pth_yield>(I<target>)
